...
def callback():
    ...
    return redirect(session.pop('url_to_return_to', url_for('home', _external=True)))
...
def requires_auth(view_function):
    ...
    def decorated(*args, **kwargs):
        if 'profile' not in session:
            # this won't work for blueprint view functions
            session['url_to_return_to'] = url_for(view_function.__name__)
